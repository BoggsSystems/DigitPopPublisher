const digitPop = (() => {
	...
	const injectButton = (assetId) => {
		...
		const projectId = "6549b90049312800020d3b80"; // Replace with the actual static project ID
		...
		btn.addEventListener("click", () => {
			...
			// 	.then((data) => {
			// 		console.log("Data returned from engagement creation:", data); // This will print the data

			// 		if (data && data._id && data.campaign) {
			// 			// Use _id for the engagement ID
			// 			launchPlayerInModal(data._id, data.campaign, data.project);
			// 		} else {
	...
	function createEngagementFromPlayer(adId) {
		const apiUrl = "http://localhost:9000"; // Replace with your actual API URL
	...
	function launchPlayerInModal() {
		...
		playerIframe.src = `http://localhost:4201/ad/6549b90049312800020d3b80`; // Replace with the actual URL of the player
    ...
    //console.log(`http://localhost:4201/ad/${projectId}/engagement/${engagementId}/campaign/${campaignId}`);
